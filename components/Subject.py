...
class Subject:
    ...
    def setupInstructors(self):
        ...
        model.setHorizontalHeaderLabels(['شناسه', 'دسترسی', 'نام'])
        tree.setModel(model)
...
# Model To Show Subject 
class Tree:
    def __init__(self, tree):
        self.tree = tree
        self.model = model = QtGui.QStandardItemModel()
        model.setHorizontalHeaderLabels(['شناسه', 'کد درس', 'نام', 'نوع', 'اساتید', 'عملکرد'])
        tree.setModel(model)
        tree.setColumnHidden(0, True)
        self.display()
    ...
    def delete(self, id):
        ...
        confirm.setText('آیا میخواهید درس انتخابی را حذف کنید')
